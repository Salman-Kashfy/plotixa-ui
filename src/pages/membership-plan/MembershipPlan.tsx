...
function MembershipPlan() {
    ...
    const btn = {
        to: ROUTES.MEMBERSHIP_PLAN.CREATE,
    ...
    const columns = [
        ...
        { id: 'isChampion', label: 'Scope', minWidth: 170 },
        { id: 'championType', label: 'Coverage', minWidth: 100 },
    ...
    useEffect(() => {
        breadcrumbContext.setBreadcrumb([{ name: 'Membership Plans' }]);
    }, []);
    ...
    const fetchRows = () => {
        ...
        GetMembershipPlans({ page: page + 1 }, params)
            .then((membershipPlans: any) => {
                const { list, paging: nextPaging } = membershipPlans;
                const nextRows = list.map((e: any) => ({
                    id: e.id,
                    name: e.name,
                    planGroup: e.group.name,
                    isChampion: <Chip label={e.isChampion ? 'Global' : 'Local'} />,
                    championType: e.championType || '-',
                    actions: (
                        <>
                            <IconButton
                                component={NavLink}
                                to={ROUTES.MEMBERSHIP_PLAN.VIEW(e.id)}
                                color="info"
                            >
                                <VisibilityIcon />
                            </IconButton>
                            {hasPermission(PERMISSIONS.MEMBERSHIP_PLAN.UPSERT) ? (
                                <IconButton
                                    component={NavLink}
                                    color="warning"
                                    to={ROUTES.MEMBERSHIP_PLAN.EDIT(e.id)}
                                >
                                    <ModeEditIcon />
                                </IconButton>
                            ) : null}
    ...
    return (
        <>
            <PageTitle title="Membership Plans" to={ROUTES.MEMBERSHIP_PLAN.CREATE} btn={btn} />
            <Card sx={{ mb: 3 }}>
                <CardContent sx={{ p: 3 }}>
                    <Grid container spacing={2}>
                        <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                            <SearchForm callback={handleSearch} label="Search Membership Plans" />
                        </Grid>
                        {brandSelection ? (
                            <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                                <Autocomplete
